...
export interface ReadingContextPart {
	type: 'readingContext'
	readings: {
		id: string
		title: string
...
// ReadingContext
export const ReadingContextPartDefinition: PromptPartDefinition<ReadingContextPart> = {
	type: 'readingContext',
	priority: -64,
	buildContent: ({ readings }) => {
		if (!readings.length) {
			return [
				'No readings are currently uploaded. Ask the user to upload readings if they want source-grounded maps.',
			]
		}

		const lines: string[] = [
			'Use these uploaded readings as source material for synthesis. Ground nodes/edges in this context:',
		]
		for (const reading of readings) {
			const createdAt = new Date(reading.createdAt).toISOString()
			lines.push(`Reading: ${reading.title} (${createdAt})`)
